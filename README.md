...
Windows Server と内部ロードバランサーを含む業務システム基盤を整理した PoC ケーススタディです。
...
- 実務由来の構成を、説明しやすいドキュメントと CloudFormation サンプルへ整理する
...
| 環境 | 構成図 | 内容 |
| --- | --- | --- |
| 検証環境 | [diagrams/aws-windows-ilb-poc-stg-architecture.png](diagrams/aws-windows-ilb-poc-stg-architecture.png) | 接続経路、最小サーバー構成、Single-AZ RDS を確認する環境 |
| 本番環境 | [diagrams/aws-windows-ilb-poc-prod-architecture.png](diagrams/aws-windows-ilb-poc-prod-architecture.png) | 2AZ、NAT Gateway x2、Linux / Windows Server x2、RDS Multi-AZ を想定した環境 |
...
| テンプレート | 内容 |
| --- | --- |
| [templates/stg-windows-ilb.yaml](templates/stg-windows-ilb.yaml) | 検証環境をもとにした VPC、Subnet、NAT Gateway、EC2、Internal ALB、RDS、Security Group |
...
| ファイル | 内容 |
| --- | --- |
| [parameters/stg.example.json](parameters/stg.example.json) | 検証環境向けのサンプルパラメータ |
| [parameters/prod.example.json](parameters/prod.example.json) | 本番環境向けのサンプルパラメータ |

実際にデプロイする場合は、サンプルファイルをコピーして使用します。

```bash
cp parameters/stg.example.json parameters/stg.json
cp parameters/prod.example.json parameters/prod.json
```

`parameters/stg.json` と `parameters/prod.json` には、自分の環境に合わせた値を設定します。環境固有の値を含むため、GitHub にはコミットしない運用とします。
...
| --- | --- | --- | --- |
| `ProjectName` | `aws-windows-ilb-poc` | `aws-windows-ilb-poc` | リソース名の接頭辞。3〜19文字、小文字英数字とハイフンのみ |
| `EnvironmentName` | `stg` | `prod` | 環境識別子 |
| `AvailabilityZoneA` | `ap-northeast-1a` | `ap-northeast-1a` | A側の Public / Private App / Private DB Subnet を配置するAZ |
| `AvailabilityZoneC` | `ap-northeast-1c` | `ap-northeast-1c` | C側の Public / Private App / Private DB Subnet を配置するAZ |
...
`ProjectName` は、ALB 名と Target Group 名の 32 文字制限に収まるよう 19 文字以下に制限しています。

`DbMasterPassword` は 8〜128 文字で、英数字と `!#$%&()*+,.:;<=>?[]^_{|}~-` を使用できます。実運用では Secrets Manager など安全な管理方法を利用してください。
...
├── diagrams
│   ├── README.md
│   ├── aws-windows-ilb-poc-prod-architecture.png
│   └── aws-windows-ilb-poc-stg-architecture.png
├── docs
│   ├── architecture.md
│   └── operations.md
├── examples
...
`parameters/stg.json` と `parameters/prod.json` の以下の値を、自分の環境に合わせて変更します。
...
aws cloudformation validate-template \
  --template-body file://templates/stg-windows-ilb.yaml
...
aws cloudformation create-stack \
  --stack-name aws-windows-ilb-poc-stg \
  --template-body file://templates/stg-windows-ilb.yaml \
  --parameters file://parameters/stg.json
```
